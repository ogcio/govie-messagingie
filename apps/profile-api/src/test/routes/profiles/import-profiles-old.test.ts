...
// This is a duplicate from import-profiles.test.ts
// We need to maintain this "old route" until we don't upgrade SDKs and everybody will use the new route
// TODO: Remove this old tests once all clients have migrated to the new route through SDK
describe("POST /api/v1/profiles/import-profiles", () => {
  ...
  describe("Valid requests", () => {
    ...
    it("should accept PPSN-only profiles with import_type=ppsn-only", async () => {
      ...
      const response = await app.inject({
        method: "POST",
        url: "/api/v1/profiles/import-profiles?importType=ppsn-only",
        payload: {
