...
- **Public-servant only** — sign-in calls `signIn()` without a `connector`, so SAG forwards a plain Logto sign-in. Logto serves its own chooser screen, matching the legacy `messaging-admin` UX. Before navigating, the app writes a `connectorsToShow=ogcio-entraid` cookie on the shared parent domain (e.g. `.dev.services.gov.ie`); Logto's chooser reads it and renders only the EntraID button — public servants never see MyGovID. This also avoids silent re-auth through Microsoft's still-active session immediately after sign-out.
...
3. **Sign-in** calls `signIn()` with no `connector`. Before that, the shell writes a `connectorsToShow=ogcio-entraid` cookie on the shared parent domain so Logto's chooser only shows the EntraID button. SAG forwards a plain Logto signIn (no `direct_sign_in`), Logto serves its own chooser, and the user clicks **Sign in with EntraID** — exactly like the legacy `messaging-admin` flow.
